			...
			<div class="col-lg-12 col-md-12">
				<section class="tile color transparent-black">
					...
					<div class="tile-body">
						...
						{{ Form::open(array('url' => '/candidate', 'method' => 'post', 'class' => 'form-horizontal', 'role' => 'form', 'parsley-validate' => '', 'id' => 'basicvalidation', 'enctype' => 'multipart/form-data')) }}

							<div class="form-group">
								<label for="student" class="col-sm-4 control-label">Student *</label>
								<div class="col-sm-6" id="select-student">
									<select name="studentid" class="chosen-select chosen-transparent form-control" id="student" parsley-trigger="change" parsley-required="true" parsley-error-container="#select-student">
										<option value="">Please choose</option>
										@foreach($students as $student)
											<option value="{{ $student->id }}" <?php if(Input::old('studentid') == $student->id) echo 'selected'; ?>>{{ ucwords($student->fname) . ' ' . ucwords($student->mname) . ' ' . ucwords($student->lname) }}</option>
										@endforeach
									</select>
								</div>
							</div>
								...
								<div class="col-sm-6" id="select-position">
									<select name="positionid" class="chosen-select chosen-transparent form-control" id="position" parsley-trigger="change" parsley-required="true" parsley-error-container="#select-position">
										...
										@foreach($positions as $position)
											<option value="{{ $position->id }}" <?php if(Input::old('positionid') == $position->id) echo 'selected'; ?>>{{ ucwords($position->name) }}</option>
										@endforeach
									</select>
								</div>
							</div>
